# -*- coding: utf-8 -*-
"""
Created on Wed Jul  7 13:38:34 2021
Hyperparameter ...GPU could not handle 128 filters :( 
@author: steve
"""
#%% Imports...
...
def train_test_model(hparams):
    starttime = time.perf_counter()
    pzastk4 = Sequential([
        Conv2D(hparams[HP_NUM_UNITS], 3, activation='relu', input_shape=(224,224,3), name='1st'),
        Conv2D(hparams[HP_NUM_UNITS], 3, activation='relu', name='2nd', padding='valid'),
        MaxPool2D(name='pool0' ), # this is REGULARIZATION! padding can also be 'same'
        Conv2D(hparams[HP_NUM_UNITS], 3, activation='relu', name='3rd', padding='valid'),
...
#    pzastk4.compile(loss='binary_crossentropy', optimizer=Adam(lr=.001), metrics='accuracy')
    pzastk4.compile(loss='binary_crossentropy', 
                    optimizer=hparams[HP_OPTIMIZER], 
                    metrics='accuracy')
    # fit the model
    pzaHist4 = pzastk4.fit(train_batch_aug, epochs=3, batch_size=32, 
                           validation_data = valid_batch, 
                           validation_steps=len(valid_batch), 
                           callbacks=[tensorboard_callback,
                                      hp.KerasCallback(logdir, hparams)],
                           ) 
    _, accuracy = pzastk4.evaluate(valid_batch)
...
for num_units in HP_NUM_UNITS.domain.values:
  for dropout_rate in (HP_DROPOUT.domain.min_value, HP_DROPOUT.domain.max_value):
    for optimizer in HP_OPTIMIZER.domain.values:
      ...
      run(logdir+'/hparam_tuning/' + run_name, hparams)
      session_num += 1
